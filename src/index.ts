...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'CoinGecko');
}
...
// Set from gateway-injected _proxyUrl/_proxyToken. CoinGecko keyless 403s CF
// egress; route GETs through the non-CF relay when configured (returns verbatim).
let PROXY: { url: string; token: string } | null = null;

function cgFetch(url: string): Promise<Response> {
  return PROXY
    ? pwFetch(PROXY.url, {
        method: 'POST',
...
async function searchCoins(query: string) {
  ...
  const res = await cgFetch(`${BASE_URL}/search?${params}`);
  if (!res.ok) throw await httpError(res, 'CoinGecko error');
...
async function getMarketData(vsCurrency: string, order: string, limit: number) {
  ...
  const res = await cgFetch(`${BASE_URL}/coins/markets?${params}`);
  if (!res.ok) throw await httpError(res, 'CoinGecko error');
...
async function getTrending() {
  const res = await cgFetch(`${BASE_URL}/search/trending`);
  if (!res.ok) throw await httpError(res, 'CoinGecko error');
